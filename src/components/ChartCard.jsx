...
const ChartCard = ({ title, dateRange, data, color = 'bg-yellow-200', type = 'line' }) => {
  ...
  const points = data.map((point, index) => {
    const x = (index / (data.length - 1)) * (chartWidth - leftPadding - padding) + leftPadding;
      ...
      <div className="relative">
        ...
        <div className="absolute left-0 top-0 h-full flex flex-col justify-between py-10">
          {yAxisValues.map((value, index) => (
            <span key={index} className="text-xs text-gray-500 dark:text-gray-400 -translate-y-1/2">
              {formatValue(value)}
            </span>
          ))}
        </div>
        ...
        {/* X-axis labels */}
        <div className="flex justify-between mt-4 ml-12 mr-10">
